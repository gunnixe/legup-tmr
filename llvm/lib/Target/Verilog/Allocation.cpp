...
Allocation::Allocation(Module *M)
    : genericRAMs(false), module(M), dataSize(0),
      ...
      generateRTLFile("generateRTL.legup.rpt", FileError,
                      llvm::sys::fs::F_None),
      partitionFile("partition.legup.rpt", FileError, llvm::sys::fs::F_None) {
    ...
    //partitionFile << getFileHeader();
	time_t timer = time(NULL);
	partitionFile << "// Generated by TLegUp Function-Level Partitioning Tool\n"
	              << "// Date: " << ctime(&timer)
...
void Allocation::dbgPrintFunctionToRams(std::string comment) {
	errs() << comment << "\n";
	for (Module::iterator F = module->begin(), E = module->end(); F != E; ++F) {
		for (std::set<RAM *>::iterator i = functionToRams[F].begin(),
	                                   ie = functionToRams[F].end(); i != ie; ++i) {
			errs() << getLabel(F) << ":" << (*i)->getName() << "\n";
		}
	}
}
...
//FIXME - temporally added to avoid LegUp crash
//When 'LOCAL_MEMORY' directive is used, memory_controller is gone. Thus it should not add memory 
//ports to the modules. However, LegUp trys to connect memory ports for the module which does not 
//exist, then LegUp crashes with an assertion, "not find signals".
bool PropagatingSignals::functionUsesMemoryForConnection(std::string name) {
	...
	if ((*it)->isMemory() && (*it)->getName()!="memory_controller_waitrequest")
	    return true;
